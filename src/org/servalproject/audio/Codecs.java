...
public class Codecs {
	...
	// private static final Vector<ICodec> codecs = new Vector<ICodec>() {
	// {
	// add(new G722());
	...
	// }
	// };
	// private static final HashMap<Integer, ICodec> codecsNumbers;
	// private static final HashMap<String, ICodec> codecsNames;

	static {
		// final int size = codecs.size();
		// codecsNumbers = new HashMap<Integer, ICodec>(size);
		// codecsNames = new HashMap<String, ICodec>(size);
		//
		// for (ICodec c : codecs) {
		...
		// String prefs = sp.getString(Settings.PREF_CODECS,
		// Settings.DEFAULT_CODECS);
		// if (prefs == null) {
		// String v = "";
		...
		// /*
		// * moves the codec to the end of the list so we end up with
		// * the new codecs (if any) at the top and the remaining ones
		// * ordered according to the user
		// */
		// if (c != null) {
		// codecs.remove(c);
		// codecs.add(c);
	...
	// public static ICodec get(int key) {
	// return codecsNumbers.get(key);
	// }
	//
	// public static ICodec getName(String name) {
	...
	public static class Map {
		...
		Vector<ICodec> codecs;

		Map(int n, ICodec c, Vector<Integer> ns, Vector<ICodec> cs) {
			number = n;
			codec = c;
			numbers = ns;
			codecs = cs;
	...
	// Vector<AttributeField> attrs = offers.getMediaDescriptor("audio")
	// .getAttributes("rtpmap");
	...
	// }
	// if (codec != null)
	// return new Map(numbers.elementAt(index), codec, numbers,
	// codecmap);
	// else
